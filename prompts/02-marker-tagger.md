...
5b. **Unit-based matching (disambiguator)** — If the quote names no marker term but states a measurement **unit** that, in the supplied `marker_units` map, corresponds to **exactly one** offered marker, tag that marker — a unique unit is a reliable identifier (e.g. "a normal range of 5-20 mIU/L" → `fasting-insulin`, since `mIU/L` maps only to it). Record the unit string as `matched_text` with `match_type: "unit"`. Do NOT use this when the unit maps to more than one marker in `marker_units`, or for generic units (`mg/dL`, `%`, `ratio`, `ng/mL`).
