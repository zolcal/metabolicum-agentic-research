...
The Standard Medical anchor functions as a sanity check, and the council is the first and only stage that sees its numbers — the brief carries no SM numbers, only a council-only `sm_reference`, so discovery and extraction run blind to them (sections 2, 17, 19). The council dereferences `sm_reference` to load the canonical SM range file (resolved into `council/sm_alignment_reference.json`), and may also use the hand-curated `sm_anchors` table where richer population context exists, one row per marker and population context where available. The SM anchor carries `evidence_weight: 0`: it is a reference, never evidence, and never raises a grade or score. Wildly out-of-range MO recommendations get a `paradigm_divergence_flag: extreme` value and an `alignment_status` of `wider_than_envelope` or `contradictory`. This does not auto-reject — MO is expected to diverge from SM, that is the point of the paradigm — but it surfaces edge cases for review.
