...
Council consensus is a runner responsibility, not a fourth free-form agent. The runner executes the council-decider prompt across three model families and compares the JSON outputs. If at least two outputs agree on `decision`, `paradigm_assigned`, and the presence or absence of a financial conflict flag, the majority result proceeds and dissent is recorded in review notes. If fewer than two agree on those fields, the claim is quarantined with `rejection_codes: ["council_disagreement"]`. Numeric fields, evidence sub-grade, and envelope-alignment differences outside the majority are preserved for review rather than silently averaged.
