...
- All practitioner-gap work pushed to origin/main (via gh keyring; the `GITHUB_TOKEN` in `~/.secrets` is an expired classic PAT — regenerate or comment it out so plain `git push` works).

## Video relevance fix (item #3) — DONE (commits `22ba4ab`, `b129cc7`)
- **Root cause:** `collect_videos._t3_guard_passes` had a `duration >= 15min` guard that let any long video pass on a casual T3-term mention (e.g. a 21-min autism video matched free-carnitine via "carnitine"). Removed it; T3 now requires T1/T2 co-occurrence or an affinity-matched practitioner channel (strengthened by the enrichment). 2 focused tests added.
- **Efficiency fix (user-flagged):** `collect_videos` was matching ALL 982 markers incl. the 752 MO-excluded ones (which never get a packet). Now scoped to the **230 MO-supported markers** only — wave-3 went 674→54, huge speedup. video-index.json files now contain MO-supported markers only (assemble only reads those).
- **Result:** free-carnitine 23→0 fluff videos; only 2 briefs dropped to zero videos (pure fluff); 85 briefs keep real videos; all waves pass acceptance.
- **Known issue (pre-existing, unrelated):** `tests/test_hermes_asset_pipeline.py::test_assemble_brief_projects_clean_pointer_fields` fails — it asserts apob's exact practitioner list, brittle now that apob resolves to the enriched cohort. Needs making hermetic (monkeypatch registry) or asserting structure not exact values.
- All on local `main`; origin behind by the new commits (`1e8fac9`, `22ba4ab`, `b129cc7` + handover) — not yet pushed.
